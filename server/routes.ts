...
Be intelligent and helpful - if something seems unclear, make reasonable inferences based on common booking patterns.`
  ...
  try {
    ...
    console.log('Twilio service not available - credentials not configured');
  }

  // Phone webhook endpoints
  app.post("/api/phone/incoming-call", (req, res) => {
    try {
      if (!twilioService) {
        return res.status(500).send("Twilio service not configured");
      }

      const twimlResponse = twilioService.generateIncomingCallResponse();
      res.type('text/xml');
  ...
  app.post("/api/phone/recording-complete", async (req, res) => {
    try {
      const { RecordingSid, RecordingUrl, CallSid, From } = req.body;
      
      console.log('Recording completed:', { RecordingSid, RecordingUrl, CallSid, From });
      ...
      const recordingData = await twilioService.processRecording(RecordingSid, RecordingUrl);
      
      // Get the recording content for AI processing
      const recordingBuffer = await twilioService.getRecordingContent(RecordingUrl);
      
      // Note: For actual audio processing, you would need to:
      // 1. Convert audio to text using speech-to-text service
      // 2. Process with Gemini AI to extract booking details
      // 3. Store the extracted data for review
      
      // For now, we'll create a placeholder call record
      const callRecord = {
        id: Date.now().toString(),
        ...
        status: 'processed',
        extractedData: null, // Will be populated by AI processing
        createdAt: new Date().toISOString()
      };

      // Store call record (you would add this to your storage interface)
      console.log('Call record created:', callRecord);

      res.json({ success: true, callRecord });
    } catch (error) {
  ...
  app.post("/api/phone/transcription", async (req, res) => {
    try {
      ...
      if (!TranscriptionText) {
        return res.json({ success: false, error: "No transcription text" });
      }

      // Process transcription with AI to extract booking details
      try {
        const extractedData = await parseVoiceToBooking(TranscriptionText);
        
        console.log('AI extracted data from call:', extractedData);

        // Store the extracted booking data for staff review
        const callBookingDraft = {
          id: Date.now().toString(),
          callSid: CallSid,
          recordingSid: RecordingSid,
        ...
        // You would store this in your database for staff to review and convert to actual bookings
        console.log('Call booking draft created:', callBookingDraft);
  ...
  // Get call records and booking drafts
  app.get("/api/phone/call-records", async (req, res) => {
    try {
  ...
  // Phone configuration endpoint
  app.get("/api/phone/config", (req, res) => {
    try {
      const phoneNumber = process.env.TWILIO_PHONE_NUMBER;
      const isConfigured = !!(process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && phoneNumber);
      
      res.json({
        configured: isConfigured,
        phoneNumber: isConfigured ? phoneNumber : null,
        webhookUrl: isConfigured ? `${req.protocol}://${req.get('host')}/api/phone/incoming-call` : null
      });
    } catch (error) {
      console.error('Error getting phone config:', error);
      res.status(500).json({ error: "Failed to get phone configuration" });
    }
  });
